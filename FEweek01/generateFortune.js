...
function getResultComment(score){
    ...
    else if (score >= 90) return "무야호! 당신에겐 오늘 기적이 펼쳐질지도 몰라요!";
    else if (score >= 70) return "뭐든 술술 풀리는 날! 자신감 있는 아기사자가 되어보세요!!!"
    else if (score >= 50) return "평범하지만 무탈한 하루! 원래 평범함이 진짜 행복함이라는거 아시죠????"
...
function generateFortuneForAll(){
    ...
    console.log(
        `오늘의 발표자는 ... ${presenter} 님 입니다! (행운 점수 : ${highestScore}점)`
    );
}
